...
// ❌ NUNCA LOGAR — dados pessoais e sensíveis
logger.info({ cpf: user.cpf })           // ❌ CPF
logger.info({ email: user.email })        // ❌ e-mail de titular
logger.debug({ password })               // ❌ senha
logger.info({ cnpj })                    // ❌ CNPJ de clientes em logs expostos
logger.info({ content: document.content }) // ❌ conteúdo de documentos regulatórios
```
...
|---|---|---|
| `development` | `debug` | `pretty` (colorido, legível) |
| `staging` | `info` | `json` (estruturado para Datadog) |
| `production` | `warn` | `json` (mínimo necessário) |

### 8.3 Campos Obrigatórios em Logs de Aplicação

```typescript
// Todo log de request deve incluir:
{
  requestId: string    // UUID da request — rastreabilidade
  tenantId: string     // Sempre — para filtragem por cliente
  action: string       // O que aconteceu — ex: 'entity.create'
  durationMs: number   // Performance
  // NÃO incluir: dados pessoais, senhas, tokens, conteúdo de docs
}
```
